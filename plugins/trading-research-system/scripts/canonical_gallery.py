    ...
    *,
    ...
    try:
        ...
        for spec in APPROVED_CAPTURES:
            ...
            if metadata["identity"].get("dailytrades-synthetic") != "true":
...
def verify_gallery(output_dir: Path) -> dict[str, object]:
    ...
    for capture in captures:
        ...
        if (
            ...
            or metadata["identity"].get("dailytrades-board") != capture.get("board")
            or metadata["identity"].get("dailytrades-view") != capture.get("view")
            or metadata["identity"].get("dailytrades-synthetic") != "true"
...
def embed_png_identity(png: bytes, *, board: str, view: str) -> bytes:
    ...
    retained = [
        ...
        if kind != b"IEND"
        and not (kind == b"tEXt" and data.partition(b"\0")[0].startswith(b"dailytrades-"))
    ]
    identity = (
        ("dailytrades-board", board),
        ("dailytrades-view", view),
        ("dailytrades-synthetic", "true"),
    )
...
def inspect_png(png: bytes) -> dict[str, object]:
    ...
    for kind, data in chunks:
        ...
        if kind == b"tEXt" and b"\0" in data:
            ...
            if key.startswith(b"dailytrades-"):
