#!/usr/bin/env python3
"""Generate one immutable canonical Portfolio Risk Board packet from a snapshot.

This entrypoint consumes a prebuilt snapshot only. It does not read a broker,
private runtime, or any network source.
"""
...
def main() -> int:
    ...
    parser.add_argument("snapshot", type=Path, help="Validated Portfolio Risk snapshot JSON")
    parser.add_argument("--output-dir", type=Path, required=True, help="Empty or byte-identical packet directory")
    args = parser.parse_args()
    try:
        snapshot = json.loads(args.snapshot.read_text(encoding="utf-8"))
        if not isinstance(snapshot, dict) or snapshot.get("board") != "portfolio_risk":
