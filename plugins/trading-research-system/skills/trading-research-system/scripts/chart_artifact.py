#!/usr/bin/env python3
"""Generate canonical Lightweight Charts PA artifacts from normalized OHLCV.

Interactive HTML rendered by TradingView Lightweight Charts v5.2.0 is always
produced. The handcrafted SVG is available only as an explicit no-browser
fallback through ``--display-output``. Durable manifest saves are opt-in only.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        "-o",
        help="Canonical Lightweight Charts HTML path; defaults to a transient .scratch artifact",
    )
    parser.add_argument(
        "--display-output",
        help="Optional handcrafted SVG path used only as a no-browser fallback",
    )
    parser.add_argument(
        "--title",
        default=None,
...
def load_lightweight_charts_script() -> str:
    if not LIGHTWEIGHT_CHARTS_ASSET.is_file():
        raise SystemExit(
            "bundled Lightweight Charts asset missing: "
            f"{LIGHTWEIGHT_CHARTS_ASSET}"
        )
    return LIGHTWEIGHT_CHARTS_ASSET.read_text(encoding="utf-8").replace(
        "</script", "<\\/script"
    )
...
<head>
  ...
  <title>{title}</title>
  <meta name="generator" content="TradingView Lightweight Charts {LIGHTWEIGHT_CHARTS_VERSION}">
  <script data-library="TradingView Lightweight Charts" data-version="{LIGHTWEIGHT_CHARTS_VERSION}">
{load_lightweight_charts_script()}
  </script>
  <style>
    :root {{
      color-scheme: light;
...
def main() -> None:
    ...
    html_output = (
        Path(args.output)
        if args.output
        else default_display_output(repo_root, artifact_id).with_suffix(".html")
    )
    write_text_artifact(html_output, render_html(payload))
    print(f"html {html_output}")

    display_output = Path(args.display_output) if args.display_output else None
    if display_output is not None:
        write_text_artifact(display_output, render_svg(payload))
        print(f"fallback {display_output}")
