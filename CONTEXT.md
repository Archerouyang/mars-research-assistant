...
**原始事件证据**：可进入研究结果的事件必须携带 `official_calendar`、`government_or_regulatory_filing`、`official_announcement` 或 `company_ir` 的 `evidence_kind`，并有 `primary_source_confirmed=true`。Web Search 仅发现候选资料，不能成为事件事实。
_Avoid_: 聚合新闻 URL、仅校验 `https` 的来源
