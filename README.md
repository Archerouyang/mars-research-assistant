...
![火星投研助手产品封面](docs/assets/readme/mars-research-assistant-hero.png)
...
一眼看到利率、通胀、市场风格、风险偏好和下周事件如何共同影响风险资产。它的重点不是罗列指标，而是给出当前市场在交易什么、需要什么证据才能改变判断。
...
- **查看自己的持仓**：在你明确同意后，读取 IBKR 持仓与现金事实，不附加风险评分或交易建议。
...
助手会优先使用语义已经验证的 IBKR 市场字段，并以公开权威来源补齐和核验。你会得到一份宏观研究面板，聚焦利率、波动率期限结构、市场风格、流动性和重要政策事件，而不是一串无关紧要的指标。
...
把公司逻辑和价格结构放在同一处。你看到的不是单一的买卖点，而是当前结构、不同情景与什么条件会推翻原有判断。
...
- 券商路径只支持 IBKR；精确 IBKR 字段不可用时，使用公开权威来源和 Web Search 定位后的权威页面兜底。
- Web Search 只负责找到来源，搜索摘要不会直接成为面板数值。
