...
uv run --offline --no-python-downloads --no-sync python "$ROOT/scripts/verify_mars_skills.py"
uv run --offline --no-python-downloads --no-sync python -m unittest discover -s "$ROOT/tests" -p 'test_*.py'
