#!/usr/bin/env python3
"""Probe one canonical startup-status surface paired with its status fixture."""

from __future__ import annotations

import argparse
from pathlib import Path


REPO = Path(__file__).resolve().parents[1]
SKILL = REPO / "skills" / "trading-research-system"
EXPECTED = SKILL / "assets" / "fixtures" / "expected"

TextContract = tuple[Path, tuple[str, ...]]

STATUS_CONTRACTS: dict[str, TextContract] = {
    "ready": (
        EXPECTED / "daily-ops-orchestrator-start-today.md",
        ("| startup_status | ready |", "runtime_origin | explicit_argument"),
    ),
    "partial": (
        EXPECTED / "daily-ops-weekend-first-start.md",
        ("| startup_status | partial |", "| formal runtime | available |"),
    ),
    "uninitialized": (
        EXPECTED / "daily-ops-weekend-empty-runtime-first-start.md",
        ("| startup_status | uninitialized |", "| formal runtime | missing |"),
    ),
}

SURFACE_CONTRACTS: dict[tuple[str, str], TextContract] = {
    ("router", "ready"): (
        SKILL / "SKILL.md",
        ("Render `runtime_dir`, `runtime_origin`, `formal runtime`, `startup_status`",),
    ),
    ("router", "partial"): (
        SKILL / "SKILL.md",
        ("Render `runtime_dir`, `runtime_origin`, `formal runtime`, `startup_status`",),
    ),
    ("router", "uninitialized"): (
        SKILL / "SKILL.md",
        ("startup_status=uninitialized", "Exact Weekend First Start"),
    ),
    ("reference", "ready"): (
        SKILL / "references" / "daily-ops-orchestrator.md",
        ("ready / partial / uninitialized", "startup completeness axis"),
    ),
    ("reference", "partial"): (
        SKILL / "references" / "daily-ops-orchestrator.md",
        ("startup_status=partial", "formal runtime"),
    ),
    ("reference", "uninitialized"): (
        SKILL / "references" / "daily-ops-orchestrator.md",
        ("ready / partial / uninitialized", "`startup_status=partial` or `uninitialized`"),
    ),
    ("template", "ready"): (
        SKILL / "assets" / "templates" / "daily-ops-orchestrator.md",
        ("| startup_status |  |  |", "### 运行状态检查"),
    ),
    ("template", "partial"): (
        SKILL / "assets" / "templates" / "daily-ops-orchestrator.md",
        ("startup_status=partial / uninitialized", "可用研究摘要"),
    ),
    ("template", "uninitialized"): (
        SKILL / "assets" / "templates" / "daily-ops-orchestrator.md",
        ("startup_status=partial / uninitialized", "缺失确认"),
    ),
    ("fixture", "ready"): STATUS_CONTRACTS["ready"],
    ("fixture", "partial"): STATUS_CONTRACTS["partial"],
    ("fixture", "uninitialized"): STATUS_CONTRACTS["uninitialized"],
    ("direct-activation", "ready"): (
        SKILL / "references" / "workflow-weekly-trading-plan.md",
        ("`startup_status`", "Preserve the deterministic values"),
    ),
    ("direct-activation", "partial"): (
        SKILL / "references" / "workflow-weekly-trading-plan.md",
        ("partial or uninitialized runtime", "Daily Ops Orchestrator"),
    ),
    ("direct-activation", "uninitialized"): (
        SKILL / "references" / "workflow-weekly-trading-plan.md",
        ("startup_status=uninitialized", "Daily Ops Orchestrator"),
    ),
}

# Every key resolves both its surface rules and a concrete status-bearing fixture.
SURFACES: dict[tuple[str, str], tuple[TextContract, TextContract]] = {
    key: (surface_contract, STATUS_CONTRACTS[key[1]])
    for key, surface_contract in SURFACE_CONTRACTS.items()
}


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--surface", required=True)
    parser.add_argument("--status", required=True)
    args = parser.parse_args()
    key = (args.surface, args.status)
    if key not in SURFACES:
        raise SystemExit(f"unsupported startup surface/status pair: {key!r}")

    for path, required_terms in SURFACES[key]:
        content = path.read_text(encoding="utf-8")
        missing = [term for term in required_terms if term not in content]
        if missing:
            raise SystemExit(
                f"{args.surface}/{args.status} missing {missing!r} in {path}"
            )
        if "formal runtime 为 partial / uninitialized" in content:
            raise SystemExit(
                f"{args.surface}/{args.status} mixes runtime and startup taxonomies"
            )

    print(f"startup surface {args.surface}/{args.status} ok")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
