...
| Longbridge OHLCV adapter | Fixture-backed | `plugins/trading-research-system/scripts/longbridge_ohlcv_adapter.py`; `plugins/trading-research-system/scripts/verify_longbridge_ohlcv_adapter_selftest.py`; saved Longbridge kline JSON normalizes to standard OHLCV JSON for `price_action_rollforward.py` without live broker reads or order actions |
...
| Setup row preparation | Fixture-backed | `plugins/trading-research-system/scripts/prepare_setup_rows.py`; `plugins/trading-research-system/scripts/verify_prepare_setup_rows_selftest.py`; `plugins/trading-research-system/scripts/verify_prepare_setup_rows_contract.py`; consumes user-confirmed setup JSON and writes scanner-ready `trade-plans.csv` / `intraday-watchlist.csv` rows |
