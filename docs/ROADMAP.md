...
| Macro panel runtime preparation | Fixture-backed | `plugins/trading-research-system/scripts/prepare_macro_panel.py`; `plugins/trading-research-system/scripts/verify_prepare_macro_panel_selftest.py`; saved macrodata JSON writes the dated runtime `macro-panel.json`, while missing input produces next-step guidance instead of invented macro reads |
