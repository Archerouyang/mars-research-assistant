...
| Weekly plan skill | deep Active Market Plan update and next-week plan | present with KVN input-read contract | Read broad sources, imported KVN snapshots, then emit compact regime, events, trade plan preparation, cross-section candidates, themes, and setup deltas. |
...
| Macro/equity skill | macro/rates filtering, thesis verification, screening | present with KVN input-read contract | Read many sources, including Longbridge `macrodata` and imported KVN snapshots when available, reject noise, and return Trade Plan Preparation inputs plus Cross-Section Candidate Pool candidates. |
| Momentum leaderboard skill | daily KVN momentum leaderboard display and query | present | Keep it thin: route to runtime health, `momentum-leaderboard.md`, and `kvn_leaderboard.py`; use outputs as research priority only. |
| Portfolio risk skill | exposure, sizing, and position daily report review | present with broker-live fixture coverage | Output should highlight only material concentration and constraint breaches. |
...
| Templates | market plan, trading profile, weekly plan, daily tracking, watchlist, trade plans, intraday watchlist, broker views, report snapshots, reviews | present with Active Market Plan fixture package | Use fixture-backed setup fields before building heavier intraday scan logic. |
| Scripts | daily init, watchlist score, portfolio risk, trade stats, append review, KVN snapshot import/query | present with fixture-package verifier | Add position daily report contract/script or intraday scan script against fixture data. |
...
Next, define the position daily report contract/script or implement `intraday_scan.py` against the fixture package now that setup pool fields are stable enough for a first scanner.
