...
| DBG-20260722-01 | P2 | closed | A decision-bearing portfolio response with usable holdings, cash, and leverage data returned prose and a table instead of the required Portfolio Risk panel. | The delivery contract routes every usable visual through `standalone_board`, and the focused regression requires a Board packet on the first visual response. | `codex/standalone-board-only` |
| DBG-20260722-02 | P1 | closed | A later Portfolio panel manually wrapped the host-dependent fragment in a temporary HTML page, producing browser-default controls and a view that differed from the accepted panel. | The public inline surface is removed; standalone HTML embeds design tokens and component styles, preserves the frozen view order, writes snapshot/HTML/manifest together, and passes direct-open interaction smoke. | `codex/standalone-board-only` |
| DBG-20260723-01 | P1 | verified | A Macro Board was assembled from ad hoc Longbridge, official, and searched values without a predeclared field contract; source labels were revised after collection, exact provider field paths were not preserved, and unavailable exact indices were handled through narrative proxy disclosure. | Macro Preflight now uses exact eligible broker market/macro records with field-level direct-public fallback, rejects every unresolved required field, and returns one user-visible blocker rather than a proxy Board. | `codex/mars-guided-research-flow` |
| DBG-20260724-01 | P1 | verified | First-run capability discovery was incorrectly coupled to account read-only consent, so a safe Longbridge/IBKR link check could be skipped or reported as `dry-run`. | Capability discovery always runs first and only checks Longbridge `check --format json` plus host-visible IBKR tools; it never reads accounts, holdings, balances, orders, credentials, or market payloads. Focused regression and Skill gate pass. | `codex/mars-guided-research-flow` |
| DBG-20260724-02 | P1 | verified | The consent-gated capability check did not bridge the current Codex task's Interactive Brokers MCP visibility, incorrectly classifying a visible IBKR plugin as unavailable. | The probe receives only current task tool names, recognizes the Interactive Brokers MCP prefix without calling any IBKR endpoint, and regression cases cover visible, absent, and unrelated tool names. | `codex/mars-ibkr-task-bridge` |
| DBG-20260724-03 | P1 | verified | A requested Macro Panel treated the direct-public capture as the only field path, bypassing eligible Longbridge/IBKR market or macro data and conflating a missing saved snapshot with current acquisition. | Capability checks precede Macro; each field uses an exact eligible broker market/macro record when available, otherwise its exact public fallback. Missing identity, native path, unit, timestamp, or close/reference-period basis returns one Data Acquisition Blocker; successful fields render the canonical standalone Board. | `codex/mars-guided-research-flow` |
| DBG-20260724-04 | P1 | verified | Fresh chats treated the Daily Ops order as advisory: they skipped the IBKR/Longbridge capability foundation, emitted prose instead of the frozen Macro Board, and entered individual/PA work after a baseline without a user request. | The phase gate requires capability-only IBKR/Longbridge checks, then the Macro Board or one blocker. It asks the user whether to show consented default-broker holdings or research a named ticker. Holdings render factual fields only; a named ticker runs the complete research bundle and 4H PA Board. Focused regression and release checks pass. | `codex/mars-guided-research-flow` |
| DBG-20260725-01 | P1 | verified | A failed legacy Treasury XML URL caused Daily Ops to return a Macro Blocker before attempting the public Web Search fallback, even though the official Treasury Daily Rates page exposed the exact 2Y, 10Y, and 30Y fields. | The source contract uses the directly readable Treasury TextView page; the source-adapter regression rejects the legacy XML route. The Skill requires Web Search discovery plus direct authority-page opening before a Blocker and requires user-visible fallback disclosure. | `dev` |
