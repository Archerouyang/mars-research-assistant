...
| DBG-20260724-04 | P1 | reopened | Fresh chats still treated the Board-first route as advisory: they emitted prose macro summaries, escalated missing option Greeks/multipliers into an IBKR fallback, began GOOGL option analysis from a classification explanation, and bypassed the frozen Macro/Portfolio renderers with non-canonical output. | The executable phase gate must bind every unscoped transition: Macro Board/Blocker first through `ResearchResult(adapter=macro)`; broker authorization next; `ready` or `option_overlay_partial` Risk Board through `ResearchResult(adapter=portfolio)`; only an explicitly user-named ticker can enter individual research or PA. A partial option overlay stays visible and out of delta/stress arithmetic without another-broker request. Focused regression and fresh-chat acceptance must pass. | `codex/mars-board-enforcement` |
