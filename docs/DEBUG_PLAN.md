...
| DBG-20260724-04 | P1 | verified | An unscoped Daily Ops start produced prose after public macro and authorized holdings, then offered an unsolicited DRAM Price Action path without first delivering Macro and Portfolio Risk Boards. | The unscoped route yields Macro Board or Blocker first, then read-only authorization, then Portfolio Risk Board or a concrete portfolio-data gap; individual research and PA occur only after the user names a ticker. Routing regression and Skill gate pass. | `codex/mars-daily-ops-baseline-panels` |
