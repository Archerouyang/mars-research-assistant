...
| DBG-20260724-01 | P1 | verified | First-run runtime health labels unprobed broker capability as `dry-run` and `needs_review`; existing local read-only authorization is never queried. | Explicit consent gates discovery; Longbridge invokes only `check --format json`; IBKR accepts only a task-visible capability result; no consent yields `authorization_pending`; focused regression and Skill gate pass. | `codex/mars-capability-probe` |
...
| DBG-20260724-03 | P1 | verified | A requested Macro Panel with complete direct-public field capture is blocked when no saved macro snapshot or private broker configuration exists, and the response asks for persistence before attempting current public acquisition. | A complete direct-public capture produces one transient standalone Board without broker probe, runtime read, or runtime write; an incomplete capture produces only the Data Acquisition Blocker; saving remains a separate post-Board confirmation. Focused regressions and the Skill gate pass. | `codex/mars-public-macro-first` |
| DBG-20260724-04 | P1 | reopened | Fresh chats treated the Daily Ops order as advisory: they skipped the IBKR/Longbridge capability foundation, emitted prose instead of frozen Boards, read holdings without an explicit Portfolio Risk request, and entered individual/PA work after a baseline without a user request. | The executable phase gate must bind every unscoped transition: capability-only IBKR/Longbridge check first; Macro Board/Blocker through `ResearchResult(adapter=macro)`; explicit user decision before any holdings read; `ready` or `option_overlay_partial` Risk Board through `ResearchResult(adapter=portfolio)`; then ask whether the user wants company research, PA, or trade guidance. A partial option overlay stays visible and out of delta/stress arithmetic without another-broker request. Focused regression and fresh-chat acceptance must pass. | `codex/mars-capability-macro-portfolio-consent` |
