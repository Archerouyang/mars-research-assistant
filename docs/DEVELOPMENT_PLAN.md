...
| P1 | planned | Add Longbridge macrodata adapter | Makes macro/rates and financial-condition reads use the Longbridge skill's `macrodata` capability when available, instead of falling back to IBKR market data or web-only commentary. | Keep this separate from broker account reconciliation; write a source-health check and a standard macro panel view for 10Y, 30Y, credit, USD, oil, gold, and liquidity inputs. |
