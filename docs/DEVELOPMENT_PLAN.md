...
| P1 | in_progress | PA OHLCV rollforward | Adds `price_action_rollforward.py` as the local bridge from authorized/user-provided OHLCV into a consistent rolling PA note before agent judgement. | Use `verify_price_action_rollforward_selftest.py`; next forward-test with a real authorized OHLCV payload in a trading research chat. |
...
| P1 | in_progress | Add broker-live position daily report | Gives the user a scheduled holdings/risk summary similar to broker-native reminders while preserving the no-trade-record-spreadsheet boundary. | Current script summarizes concentration, instrument/product exposure, broker/account exposure, cash, and leveraged ETF path risk from `portfolio_snapshot.csv`; next connect Longbridge skill/Terminal CLI and IBKR connector adapters to the standard broker-live runtime view. |
