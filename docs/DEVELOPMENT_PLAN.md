...
| P2 | in progress | Complete the canonical Macro Regime Board (#56) | Adds a plan-linked Macro Board with five evidence-gated views, Decision Cascade, holding-first Exposure Lens, event scenario playbook, immutable synthetic snapshots, and bundled offline ECharts. | Implement only the new canonical Macro Board entrypoint alongside the legacy static SVG path; run focused artifact/browser acceptance and existing plugin gates before review. |
