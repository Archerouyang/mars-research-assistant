...
| P1 | in_progress | Add broker-live position daily report | Gives the user a scheduled holdings/risk summary similar to broker-native reminders while preserving the no-trade-record-spreadsheet boundary. | Next: connect Longbridge skill/Terminal CLI and IBKR connector adapters to the standard broker-live runtime views after `position_daily_report.py` and `automation-position-daily-report.md` fixture paths are stable. |
