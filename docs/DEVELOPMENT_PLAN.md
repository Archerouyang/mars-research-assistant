...
| --- | --- | --- | --- | --- |
| P0 | review | Release Skill-only 火星投研助手 1.0.0 | Completes the approved hard rename and field-first Macro delivery without restoring a duplicate Plugin identity or deprecated Portfolio Risk flow. | Review `docs/MARS_RESEARCH_ASSISTANT_1_0_SPEC.md` against current `dev`, then approve promotion to `master` only after release acceptance. |
| P0 | done | Simplify Mars Daily Ops into Macro, optional Holdings Display, and named-instrument research | The mandatory Portfolio Risk phase conflated broker market data with account data and blocked explicit research requests. | The approved [guided research flow spec](specs/2026-07-25-mars-guided-research-flow.md) is implemented: field-level Macro preflight, per-request holdings consent, named-instrument 4H PA, and retired Mars-only Portfolio Risk logic all pass the Skill gate. |
...
| P2 | deferred | Public visual cutover | Keeps accepted private standalone Boards separate from the paused public Board/gallery release. | Proceed only after explicit release-candidate approval. |
