...
| P2 | review | Complete the canonical Portfolio Risk Board (#59) | Adds six source-reconciled Portfolio views with explicit NAV-only separation, look-through/product/currency risk, exclusions, and additive stress scenarios. | PR #68 targets `dev`; final review fixes keep scenario-only gaps visible in the static Evidence rail and require auditable FX normalization for cross-currency NAV context. |
