...
| P1 | in_progress | Add broker-live position daily report | Gives the user a scheduled holdings/risk summary similar to broker-native reminders while preserving the no-trade-record-spreadsheet boundary. | Extend `position_daily_report.py` from fixture CSV rendering to broker-source adapters and automation prompt. |
