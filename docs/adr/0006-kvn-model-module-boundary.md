...
- Put KVN factor calculation directly inside the Mars Research Assistant Skill.
- Keep KVN entirely outside the project and only allow manual CSV import.
- Define a separate KVN model module contract, then let the Skill consume the
...
historical Top10 memory, model versioning, and validation. The Skill is
...
Until that implementation exists, the Skill must continue to treat KVN as an
imported or upstream snapshot source.

The KVN module does not need to run on the same machine as the Skill. It may run
as a local batch job, cloud scheduled job, GitHub Action, managed container, or
read-only model API, provided the Skill receives the same standardized
...
- The Skill must not calculate, re-rank, re-score, or relabel KVN rows.
...
- Cloud deployment is allowed, but the Skill should cache imported/fetched
  snapshots into the private runtime before using them so research notes remain
  reproducible.
- Any cloud API must be read-only from the Skill's perspective and must not
  make the Skill responsible for model scoring, vendor data licensing, or
  backtest governance.
