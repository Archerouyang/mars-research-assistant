...
1. 默认在活动工作目录的 `mars-research/` 创建唯一的
   `YYYY-MM-DD-SYMBOL-deep-research[-NN].md`；用户显式指定其他本地目录时使用该目录。绝不覆盖
   既有文件，也不写入安装目录。
...
4. 只有来源可追溯 JSON 同时具有价格、股本、净债务、WACC、永续增长率及三情景 FCF 路径时，
   才运行包内 `scripts/render_deep_equity_research.py` 计算 DCF；路径标识必须为 `bear`、`base`、
   `bull`。缺少任一输入则保留估值章，写明数据缺口，不用模型记忆补数。反向 DCF 还要求带来源的
   当前 FCF 与 1–20 年整数预测期。
