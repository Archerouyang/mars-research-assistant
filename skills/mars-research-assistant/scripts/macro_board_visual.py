...
def _metric_card(
    ...
    return (
        ...
        '<small>来源：{source} · 截至：{as_of}</small></article>'
    ).format(
        label=escape(label),
...
def _signed_change(stat: Mapping[str, Any]) -> str:
    ...
    if percent is None:
        return f"30 日{direction} {_number(abs(change))}"
    return f"30 日{direction} {_number(abs(change))} ({abs(float(percent)):.1f}%)"
...
def _event_row(event: Mapping[str, Any]) -> str:
    return (
        ...
        '<p>{transmission}</p><p class="evidence-reading">证据类型：{evidence_kind}'
        ' · 已确认一手来源：{primary_source_confirmed}</p>'
        '<p class="module-meta">来源：{source} · 截至：{as_of}</p></div></article>'
    ).format(
        category=escape(str(event["category"])),
        status=escape(_event_status(event["status"])),
    ...
    current_status = "".join(
        (
            _status_row(
                ...
                f"来源：{sources['treasury_2y']} · 截至：{as_ofs['treasury_2y']}",
            ),
            _status_row(
                "波动与跨资产",
                ...
                f"来源：{sources['vix']} · 截至：{as_ofs['vix']}",
            ),
            _status_row(
                "相对强弱",
                ...
                f"来源：{sources['hyg_lqd_history']} · 截至：{as_ofs['hyg_lqd_history']}",
            ),
        )
    )
    exposures = (
...
<header class="masthead">
  <div><p class="eyebrow">宏观环境</p><h1>宏观环境研究简报</h1><p class="lede">冻结市场快照、相对强弱与已确认重大事件；所有交互只在本页内发生，不保存状态。</p></div>
  <dl class="provenance" aria-label="数据时点与来源"><div><dt>市场截至</dt><dd>__AS_OF__</dd></div><div><dt>国债截至</dt><dd>__TREASURY_AS_OF__</dd></div><div><dt>事件来源</dt><dd>__EVENT_SOURCE__ · __EVENT_AS_OF__</dd></div></dl>
</header>
<section class="summary" aria-label="宏观快照摘要"><article><span class="field-label">交付范围</span><strong>利率、波动率与跨资产</strong><p>只呈现冻结的 Macro v1 字段。</p></article><article><span class="field-label">相对强弱</span><strong>HYG/LQD · NDX/RUT</strong><p>30 个共同完成的 XNYS 交易日。</p></article><article><span class="field-label">交互方式</span><strong>一次生成，仅浏览器内</strong><p>页签与筛选不会持久化。</p></article></section>
<p class="decision-framing"><strong>当前快照：</strong>__DECISION_FRAME__</p>
<section class="market-strip" aria-label="利率与跨资产快照">__RATE_CARDS____CROSS_ASSET_CARDS__</section>
<nav class="view-tabs" role="tablist" aria-label="宏观环境看板视图"><button class="view-tab" type="button" role="tab" data-view="trend" aria-controls="panel-trend" aria-selected="true">趋势</button><button class="view-tab" type="button" role="tab" data-view="current" aria-controls="panel-current" aria-selected="false">当前状态</button><button class="view-tab" type="button" role="tab" data-view="events" aria-controls="panel-events" aria-selected="false">下周事件</button><button class="view-tab" type="button" role="tab" data-view="scenarios" aria-controls="panel-scenarios" aria-selected="false">情景</button></nav>
<section class="view-panel is-active" id="panel-trend" data-panel="trend" role="tabpanel"><div class="section-head"><div><p class="eyebrow">相对强弱</p><h2>相对强弱趋势</h2></div><p>30 个共同完成的 XNYS 交易日</p></div><div class="series-controls" aria-label="趋势序列"><button class="series-control" type="button" data-series="HYG/LQD" aria-pressed="false">HYG/LQD</button><button class="series-control" type="button" data-series="NDX/RUT" aria-pressed="true">NDX/RUT</button></div><div class="chart-shell"><svg class="macro-chart" role="img" aria-label="宏观相对强弱趋势"></svg><p class="trend-caption" aria-live="polite"></p></div><p class="module-meta">HYG/LQD 来源：__HYG_SOURCE__ · 截至：__HYG_AS_OF__<br>NDX/RUT 来源：__NDX_SOURCE__ · 截至：__NDX_AS_OF__</p></section>
<section class="view-panel" id="panel-current" data-panel="current" role="tabpanel" hidden><div class="section-head"><div><p class="eyebrow">当前快照</p><h2>当前状态</h2></div><p>基于当前冻结字段，不引入外部叙事。</p></div><div class="status-list">__CURRENT_STATUS__</div><div class="exposure-control"><label for="macro-exposure">宏观因子暴露</label><select id="macro-exposure" data-exposure-select>__EXPOSURE_OPTIONS__</select></div><p class="module-meta">风险暴露只描述宏观因子敏感度，不代表持仓、账户、仓位或交易建议。</p><div class="exposure-list">__EXPOSURE_ROWS__</div></section>
<section class="view-panel" id="panel-events" data-panel="events" role="tabpanel" hidden><div class="evidence-rail"><div class="section-head"><div><p class="eyebrow">一手来源证据</p><h2>重大事件</h2></div><p>未来七天及过去 24 小时。</p></div><div class="evidence-list">__EVENTS__</div></div></section>
<section class="view-panel" id="panel-scenarios" data-panel="scenarios" role="tabpanel" hidden><div class="section-head"><div><p class="eyebrow">条件框架</p><h2>情景</h2></div><p>条件框架，不是预测。</p></div><div class="scenario-grid">__SCENARIOS__</div></section>
<footer>宏观环境看板 · 一次性自包含研究交付</footer>
</main>
<script>
