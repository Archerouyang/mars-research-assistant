...
<meta name="generator" content="Mars Research Assistant Macro Regime renderer {escape(snapshot['renderer_version'])}">
...
def _market_strip(rows: list[Mapping[str, Any]]) -> str:
    cards = "".join(
        f'<article><span>{escape(row["label"])}</span><strong>{escape(_display_reading(row))}</strong></article>'
        for row in rows
    )
    return f'<div class="market-strip">{cards}</div>'


def _display_reading(row: Mapping[str, Any]) -> str:
    label = str(row["label"]).lower()
    value = row["value"]
    if label in {"2y", "10y", "30y"} or "yield" in label or "credit" in label or "cpi" in label or "ppi" in label:
        return f"{value:g}%"
    if "oil" in label:
        return f"${value:g}"
    return f"{value:g}"
...
function selectMacroView(button){{const target=button.dataset.viewTarget;macroViews.forEach((panel)=>{{panel.hidden=panel.dataset.view!==target;}});macroTabs.forEach((item)=>item.setAttribute('aria-selected',String(item===button)));if(target==='overview'&&macroChartInstance){{requestAnimationFrame(()=>macroChartInstance.resize());}}}}
...
if(chart&&window.echarts){{macroChartInstance=echarts.init(chart,null,{{renderer:'svg'}});macroChartInstance.setOption({{animation:false,aria:{{enabled:true}},grid:{{left:58,right:42,top:20,bottom:28}},tooltip:{{trigger:'axis',axisPointer:{{type:'shadow'}}}},xAxis:{{type:'value',axisLabel:{{color:'#586672'}},splitLine:{{lineStyle:{{color:'#e4e9ed'}}}}}},yAxis:{{type:'category',data:macroBoardPayload.chart_series.map((item)=>item.label),axisTick:{{show:false}},axisLine:{{show:false}}}},series:[{{type:'bar',barWidth:18,data:macroBoardPayload.chart_series.map((item)=>({{value:item.value,itemStyle:{{color:item.label==='Oil'?'#9a5b00':'#1769aa'}}}})),label:{{show:true,position:'right',formatter:(params)=>macroBoardPayload.chart_series[params.dataIndex].value}}}}]}});document.documentElement.classList.add('chart-ready');window.addEventListener('resize',()=>macroChartInstance.resize());}}
