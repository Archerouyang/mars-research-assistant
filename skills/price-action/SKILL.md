...
当合格 OHLCV 为 1D 且用户选择 FMP、明确不使用 FMP，或在 FMP 失败后明确确认切换至
yfinance 时，可在 Markdown 内嵌静态 SVG 日线图：展示最近 120 根 K 线、成交量、
SMA20/50/200 与已解析的关键位。为让 SMA200 覆盖全部可视区间，FMP 与 yfinance 都需要
至少 319 根历史日线；图表与文字必须使用同一份 OHLCV，并标记来源和 as_of。yfinance 图表
还必须显式标为非官方 best-effort。图表不创建 HTML 文件、浏览器状态或持久化研究状态。
