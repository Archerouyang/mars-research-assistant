...
</div>{render_evidence_rail(snapshot, payload['modules'], 'At least one usable position-detail source; complete additionally requires reconciled detail, product fields, mappings, and stress inputs.')}</div>
...
def _overview(payload: Mapping[str, Any], modules: Mapping[str, Mapping[str, Any]], default_id: str) -> str:
    ...
    return f"""<section class="view-panel" id="view-overview" data-view="overview" role="tabpanel" aria-labelledby="tab-overview"{_hidden('overview', default_id)}><div class="section-head"><h2>Risk Decision Ledger</h2><p>Exposure → Risk issue → Affected holdings → Plan constraint</p></div><div class="ledger">{ledger}</div><div class="section-head"><h2>Exposure Spine</h2><p>Follow one correlated risk through every decomposition</p></div><div class="spine">{spine}</div><div class="section-head"><h2>Evidence modules</h2><p>Coverage and gaps remain visible</p></div>{module_rows}{_exclusions(payload)}</section>"""
...
def _stress_view(payload: Mapping[str, Any], modules: Mapping[str, Mapping[str, Any]], default_id: str) -> str:
    ...
    rows = "".join(_stress_row(item, positions_by_id) for item in payload["stress_scenarios"])
...
def _stress_row(item: Mapping[str, Any], positions_by_id: Mapping[str, Mapping[str, Any]]) -> str:
    contributions = "; ".join(
        f'{positions_by_id[contribution["position_id"]]["symbol"]}: {_money(contribution["estimated_impact"])}'
        for contribution in item["impact_contributions"]
    )
    return f'<article class="stress-row"><div><span class="badge {escape(item["status"])}">{escape(item["confidence"])} confidence</span><h3>{escape(item["name"])}</h3></div><p><strong>Assumptions:</strong> {escape("; ".join(item["assumptions"]))}</p><dl><div><dt>Covered exposure</dt><dd>{item["covered_exposure"]:,.0f}</dd></div><div><dt>Missing exposure</dt><dd>{item["missing_exposure"]:,.0f}</dd></div><div><dt>Estimated impact</dt><dd>{item["estimated_impact"]:,.0f}</dd></div></dl><p><strong>Impact contributions:</strong> {escape(contributions)}</p><p><strong>Affected holdings:</strong> {escape(", ".join(item["affected_holdings"]))}</p><p><strong>Plan constraint:</strong> {escape(item["plan_constraint"])}</p><p class="module-meta">Position-level additive model · no diversification credit</p></article>'
...
if(stressChart&&window.echarts){{portfolioStressChart=echarts.init(stressChart,null,{{renderer:'svg'}});portfolioStressChart.setOption({{animation:false,aria:{{enabled:true}},tooltip:{{show:false}},xAxis:{{type:'value'}},yAxis:{{type:'category',data:portfolioRiskPayload.stress_scenarios.map((item)=>item.name)}},series:[{{type:'bar',data:portfolioRiskPayload.stress_scenarios.map((item)=>item.estimated_impact),itemStyle:{{color:'#b43b32'}}}}]}});document.documentElement.classList.add('chart-ready');window.addEventListener('resize',()=>portfolioStressChart.resize());}}
...
def _styles() -> str:
    return f"""
:root{{--bg:#f7f8fa;--surface:#fff;--ink:#17202a;--muted:#65717e;--line:#dce2e8;--accent:#1769aa;--danger:#b43b32;--warn:#9a6700;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:var(--ink);background:var(--bg)}}*{{box-sizing:border-box}}body{{margin:0;padding:16px;background:var(--bg);color:var(--ink)}}{shared_shell_styles()}main{{max-width:1180px;margin:auto}}.masthead,.section-head,.decision-card,.summary article,.module-row,.ledger-row,.spine-row,.aggregation-row,.product-detail,.broker-row,.stress-row,.exclusions,.evidence-rail,footer{{background:var(--surface);border:1px solid var(--line);border-radius:8px}}.masthead{{display:flex;justify-content:space-between;gap:20px;padding:20px}}h1,h2,h3,p{{margin-top:0}}.eyebrow,.module-meta,.source-time,dt{{color:var(--muted);font-size:.82rem}}.provenance{{display:grid;gap:8px;min-width:250px}}.provenance div,dl div{{display:flex;justify-content:space-between;gap:14px}}.provenance dt,.provenance dd,dl dd{{margin:0}}.view-tabs{{display:flex;flex-wrap:wrap;gap:6px;margin:14px 0}}button{{border:1px solid var(--line);background:var(--surface);padding:8px 10px;border-radius:6px;color:inherit}}button[aria-selected=true]{{background:var(--ink);color:white}}button:focus-visible{{outline:3px solid #69a7d6;outline-offset:2px}}.summary{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px}}.summary article{{padding:14px}}.summary span{{display:block;color:var(--muted)}}.summary strong{{display:block;margin-top:5px}}.decision-card{{padding:18px;margin:12px 0}}.decision-framing{{margin-bottom:0}}.board-layout{{display:grid;grid-template-columns:minmax(0,1fr) 280px;gap:14px;align-items:start}}.primary-flow,.ledger,.spine,.aggregation-list,.product-grid,.broker-list,.stress-list{{display:grid;gap:10px}}.view-panel{{display:grid;gap:10px}}.section-head{{display:flex;justify-content:space-between;gap:12px;padding:12px 14px;align-items:baseline}}.section-head h2,.section-head p{{margin:0}}.ledger-row,.module-row{{display:grid;grid-template-columns:minmax(150px,.45fr) minmax(0,1.55fr);gap:14px;padding:14px}}.spine-row,.aggregation-row,.product-detail,.broker-row,.stress-row,.exclusions{{padding:14px}}.aggregation-row dl,.product-detail dl,.broker-row dl,.stress-row dl,.fact-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:8px}}.aggregation-row dl div,.product-detail dl div,.broker-row dl div,.stress-row dl div,.fact-grid div{{display:grid;gap:2px;padding:8px;background:#f3f5f7;border-radius:6px}}.metric,.badge{{display:inline-block;padding:3px 7px;border-radius:999px;background:#e8eef5;font-size:.78rem}}.badge.high,.badge.source_error{{background:#fde7e5;color:#8f261e}}.badge.medium,.badge.partial,.badge.stale{{background:#fff0cf;color:#765000}}.badge.complete{{background:#e5f3e8;color:#246a36}}.evidence-rail{{padding:14px;position:sticky;top:10px}}.evidence-rail ul{{padding-left:18px}}.evidence-rail li{{margin-bottom:9px}}.source-time{{display:block}}.chart{{height:280px;background:var(--surface);border:1px solid var(--line);border-radius:8px}}footer{{padding:14px;margin-top:14px}}.enhanced [data-view][hidden]{{display:none!important}}@media(max-width:760px){{body{{padding:8px}}.masthead{{display:grid}}.provenance{{min-width:0}}.summary,.board-layout{{grid-template-columns:1fr}}.evidence-rail{{position:static}}.ledger-row,.module-row{{grid-template-columns:1fr}}}}@media(max-width:380px){{.summary{{grid-template-columns:1fr}}.section-head{{display:grid}}button{{flex:1 1 46%}}}}
"""
