...
Use `chart_artifact.py` for symbol-level price action reads. Its canonical
artifact is TradingView Lightweight Charts v5.2.0 interactive HTML. Capture the
inline chat image from that HTML in a browser; use the handcrafted SVG only as
an explicit no-browser fallback. The board should show:
